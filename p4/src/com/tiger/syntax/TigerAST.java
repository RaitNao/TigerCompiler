...
public class TigerAST {
    ...
    public class Node {
        ...
        public Node(Node parent, TigerNT symbol, int expectedNumCh) {
            this.parent = parent;
            this.symbol = symbol;
        ...
        public Node(Node parent, TigerToken token) {
            this.parent = parent;
            this.symbol = token;
        ...
        public void setSymbol(TigerSymbol symbol) {
            this.symbol = symbol;
        }
        ...
        public int getExpectedNumCh() {
            return expectedNumCh;
        }
        ...
        public List<TigerToken[]> getTokens() {
            List<TigerToken[]> tokenList = new ArrayList<>();
            if (this.isNT()) {
                for (Node child: children) {
                    List<TigerToken[]> childList = child.getTokens();
                    tokenList.addAll(childList);
                }
            } else {
                tokenList.add(new TigerToken[]{(TigerToken) symbol});
            }
            return tokenList;
        }
