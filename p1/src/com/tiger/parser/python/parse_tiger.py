...
def main():
    ...
    LL.export_to_java_class(JAVA_CLASS_PATH)
...
class LLGrammar(object):
    ...
    def export_to_java_class(self, path):
        if path.rfind('/') != -1:
            directory = path[:path.rfind('/')]
            if not os.path.isdir(directory):
                print "DOESNT EXIST"
...
public class LLTable {
    ...
    private static Map<String, Map<TokenType, Integer>> createMap() {
        ...
        for NT in self.ll_table:
            map_entry = ["""\n\t\tmap.put("{}", new HashMap<TokenType, Integer>() {{\n\t\t\t{{""".format(NT)]

            for term_pair in self.first_plus[NT]:
                for T in term_pair[1]:
                    if T in self.terminals:
                        pair = """\n\t\t\t\tput(TokenType.{}, {});""".format(translation[T], term_pair[0])
